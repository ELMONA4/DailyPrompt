...
function addOrUpdateActivity() {
    ...
    if (name && time && date) {
        const activityData = {
            ...
            priority,
            completed: false,
            reminderTime: reminderTime, // Asegúrate de guardar el tiempo de recordatorio
            id: Date.now()
        };
        ...
        if (editingItem) {
            ...
            scheduleNotification(name, time, date, reminderTime);
        }
...
function scheduleNotification(name, time, date, reminderTime) {
